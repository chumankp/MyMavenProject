  ...
  stages {
    ...
    stage('Build with Maven') {
      tools {
        ...
        jdk 'jdk11'   // optional: name of JDK configured in Jenkins
      }
      steps {
    ...
    stage('Push to Docker Hub') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'dockerhub-creds', usernameVariable: 'chumankp', passwordVariable: 'Kalia@720')]) {
          sh '''
